...
<Popover.Root bind:open>
 ...
 <Popover.Content class="w-full p-0 bg-transparent text-white {glass}">
  <Command.Root>
  ...
  .replace(/\b\w/g, char => char.toUpperCase())}..." />
   <Command.List>
    ...
    <Command.Group class="w-full p-0 bg-transparent text-white {glass}">
     {#each items as  item}
      <Command.Item
       value={item.name}        
    keywords={[item.name]}    
    onSelect={() => {
     ...
     class="w-full p-0 bg-transparent text-white {glass} {selectItem}"
      >
       <CheckIcon
